<script lang="ts">
	import { atUriToPostUri, renderPostAsHtml, numberToHumanReadable } from './utils';
	...
	<div class="mt-2 pb-2">
		{#if expanded}
			...
			<div class="prose prose-sm prose-neutral dark:prose-invert">
				{@html renderPostAsHtml(comment.post)}
			</div>
			...
			<div class="mt-2 flex gap-8 text-base-500 dark:text-base-400">
				...
				>
					...
					{numberToHumanReadable(comment.post.repostCount)}
				</a>
			...
			{/if}
		{:else}
			<button onclick={() => (expanded = true)} class="cursor-pointer">
				{@render top(true)}
			</button>
		{/if}
	</div>
