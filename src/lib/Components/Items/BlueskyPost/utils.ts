...
export function renderPostAsHtml(post?: AppBskyFeedDefs.PostView | Post) {
	...
	for (const segment of rt.segments()) {
		if (segment.isLink()) {
			html += `${preLink} href="${encodeURI(segment.link?.uri ?? '')}">${escapeHTML(segment.text)}</a>`;
		} else if (segment.isMention()) {
			html += `${preLink} href="https://bsky.app/profile/${encodeURI(
				segment.mention?.did ?? ''
			)}">${escapeHTML(segment.text)}</a>`;
		} else if (segment.isTag()) {
			html += `${preLink} href="https://bsky.app/hastag/${encodeURI(
				segment.tag?.tag ?? ''
